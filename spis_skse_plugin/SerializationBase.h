...
#ifndef _SERIALIZATIONBASE_H
#define _SERIALIZATIONBASE_H
...
namespace spis
{
	...
	{
	public:
		virtual void Serialize(SKSESerializationInterface * intfc, T obj) = 0;
		virtual T Unserialize(SKSESerializationInterface * intfc) = 0;
	};
	...
	{
		UInt32 vs = ev->size();
		intfc->WriteRecordData(&vs, sizeof(std::size_t));
	...
	void SerializeGroundMap(SKSESerializationInterface * intfc, typename BasicTracker<T>::GroundMap * gm, SerializeT<T>& pred)
	{
		...
		{
			SerializeGroundKey<T>(intfc, it->first);
			pred.Serialize(intfc, it->second);
		}
	}
	...
	void SerializeContainerMap(SKSESerializationInterface * intfc, typename BasicTracker<T>::ContainerMap * cm, SerializeT<T>& pred)
	{
		...
		for (auto it = cm->begin(); it != cm->end(); it++)
		{
			...
			intfc->WriteRecordData(&mss, sizeof(std::size_t));
	...
	void SerializeBasicTracker(SKSESerializationInterface * intfc, typename BasicTracker<T> * bt, SerializeT<T>& pred)
	{
		...
		SerializeEquipVector<T>(intfc, bt->ev);
	}
